...
impl Tracer {
    ...
    pub fn trace(&mut self, camera: &Camera, objects: &Vec<&Hitable>) {
        for y in 0..self.image_height {
            for x in 0..self.image_width {
                ...
                for &(sx, sy) in self.pixel_sampler.unit_square_samples.iter() {
                    ...
                    color += self.trace_ray(&ray, &objects, 0);
                }
    ...
    fn trace_ray(&self, ray: &Ray3, objects: &Vec<&Hitable>, depth: u32) -> Color {
        ...
        if let Some(intersection) = have_hit {
            ...
            if depth < self.max_trace_depth && (*intersection.material).scatter(ray, &intersection, &mut attenuation, &mut scattered) {
                return self.trace_ray(&scattered, objects, depth + 1) * attenuation;
            } else {
                return Color::black();
            }
        } else {
